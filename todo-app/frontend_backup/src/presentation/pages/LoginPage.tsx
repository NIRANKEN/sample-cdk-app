...
const LoginPage: React.FC = () => {
  ...
  useEffect(() => {
    if (isAuthenticated) {
      navigate('/todos'); // ログイン済みならTODOページへリダイレクト
    }
  }, [isAuthenticated, navigate]);

  useEffect(() => {
    // Zustandストアのエラーをローカルのフォームエラーに同期
    if (error) {
      // エラーオブジェクトからメッセージを抽出 (Cognitoエラーは name と message を持つ)
  ...
  const handleFormSwitch = (type: FormType) => {
    ...
    setNewPassword('');
    setConfirmationCode('');
    clearError(); // ストアのエラーもクリア
    setFormError(null); // ローカルのエラーもクリア
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Zustandストアでエラーがセットされるが、ローカルでもエラーメッセージを表示するためにセット
      // err.message は Zustandストアのエラーから取得されるので、ここでは特別な処理は不要
      ...
      case 'confirmSignUp':
        return (
          <>
            ...
            <input
              type="text" // emailは表示だけにするか、入力不可にする
              placeholder="Email (Verification Target)"
              value={email}
              onChange={(e) => setEmail(e.target.value)} // 基本的には変更させない想定
              disabled // 変更不可にする
              // readOnly
            />
            <input
  ...
  const inputStyle: React.CSSProperties = { // App.css でグローバルに適用することも検討
    padding: '10px',
    border: '1px solid #ddd',
    borderRadius: '4px',
  };

  const buttonStyle: React.CSSProperties = { // App.css でグローバルに適用することも検討
    padding: '10px 15px',
    border: 'none',
    borderRadius: '4px',
  ...
  return (
    <div style={pageStyle}>
      ...
      {/* 以下のようなスタイルをグローバルCSSに定義すると良いでしょう */}
      <style>{`
        input[type="email"], input[type="password"], input[type="text"] {
          padding: 10px;
          border: 1px solid #ddd;
          border-radius: 4px;
          box-sizing: border-box; /* labelなどと幅を揃えやすくする */
        ...
        button[type="button"] {
            background-color: #6c757d; /* より目立たない色に */
        }
